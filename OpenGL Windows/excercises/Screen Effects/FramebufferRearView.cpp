...
class Framebuffer_RearView : public Window {
public:
    Framebuffer_RearView(const std::string& wnd, const int& w, const int h, const json& jsonConfig)
        : Window(wnd, w, h), mainCamera()
    {
        RetrieveSettings(jsonConfig);

        initFrameBufferQuads(w, h);

        // init scene
        mainScene.InitializeGlobalData();
    }

    ~Framebuffer_RearView() {
    ...
    void RetrieveSettings(const json& jsonConfig) {
        ...
        settings.RearViewShaderFrag = 
            Assets::GetShaderCode(jsonConfig["RearViewScreenShader"]["Frag"]);
        settings.RearViewWidth = jsonConfig["RearView"]["Width"];
        settings.RearViewHeight = jsonConfig["RearView"]["Height"];

    }

    void initFrameBufferQuads(float wndWidth, float wndHeight) {
        
        // whole screen
        screenFrameBuffer = std::make_unique<FrameBufferQuad>(
            wndWidth, wndHeight, 
            new Shader(settings.ScreenShaderVertex, settings.ScreenShaderFrag)
        );

        // rear view in upper-right corner
        float minLeftRV = 1.0f - (settings.RearViewWidth / wndWidth),
            minBottomRV = 1.0f - (settings.RearViewHeight / wndHeight);
        float rearViewVerts[] = {
            // positions                // texCoords
            minLeftRV,  1.0f,           0.0f, 1.0f,
            minLeftRV,  minBottomRV,    0.0f, 0.0f,
            1.0f,       minBottomRV,    1.0f, 0.0f,

            minLeftRV,  1.0f,           0.0f, 1.0f,
            1.0f,       minBottomRV,    1.0f, 0.0f,
            1.0f,       1.0f,           1.0f, 1.0f
        };
        rearViewFrameBuffer = std::make_unique<FrameBufferQuad>(
            settings.RearViewWidth, settings.RearViewHeight,
            new Shader(settings.ScreenShaderVertex, settings.RearViewShaderFrag),
            rearViewVerts, sizeof(rearViewVerts)
        );
    }
    ...
    void OnRender() {
        ...
        screenFrameBuffer->RenderQuad(clearEverything);
    ...
    void RenderRearView() {
        ...
        // rotate and render again the whole scene
        mainCamera.RotateYaw(180.0f);
        ...
        rearViewFrameBuffer->UseAsRenderTarget();
        // render to rear view screen texture
        mainScene.Render(view, proj);
        // display the final from framebuffer WITHOUT deleting the previous render
        rearViewFrameBuffer->RenderQuad(false);
...
private:
    ...
    std::unique_ptr<FrameBufferQuad> screenFrameBuffer, rearViewFrameBuffer;
    ...
    struct Settings {
        ...
        std::string ScreenShaderVertex, ScreenShaderFrag, RearViewShaderFrag;

        float RearViewWidth, RearViewHeight;
...
int main() {
    std::ifstream jsonFile(
        Assets::GetConfigFilePath("Screen Effects/FrameBufferRearView_config.json")
    );
    json configData = json::parse(jsonFile);

    std::cout << "Current PATH => " << std::filesystem::current_path() << std::endl;

    Framebuffer_RearView wnd(
