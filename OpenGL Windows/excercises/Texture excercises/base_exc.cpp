...
const std::string SRC_VERTEX =
    ...
    "   ourColor = aColor;"
    "   texCoord = aTexCoord;"
    "   gl_Position = vec4(displacementOffset + aPos.xyz, 1.0);\n"
    "}\n";
...
int main() {
    ...
    VAO = CreateRectangle(rect, sizeof(rect), indices, sizeof(indices));
    shader.Use();
    shader.SetVec3("displacementOffset", 0.0f, 0.0f, 0);
    glBindVertexArray(VAO);
